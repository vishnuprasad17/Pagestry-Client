...
import { useUpdateAddressMutation, useFetchAddressQuery } from "../../../redux/features/address/addressApi";
...
const EditAddress: React.FC = () => {
  ...
  const { id } = useParams();
  const { data: address, isLoading: isFetching } = useFetchAddressQuery(id || "");
  ...
  const onSubmit = async (formData: EditAddressFormValues) => {
    ...
    try {
      ...
      const res = await updateAddress({ id, address: updatedData });
    ...
    <div className="min-h-screen bg-gray-50 py-6">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mt-4 text-xs text-gray-600">
          <p>By editing an address, you agree to our privacy notice and conditions of use.</p>
        </div>
      </div>
    </div>
  );
